...
class EventsContentController extends Controller
{
    ...
    {
        return view('admin.events-content.edit', [
            'content' => SiteContent::valuesFor('events', EventsContent::defaults()),
        ]);
    }
    ...
    public function update(Request $request): RedirectResponse
    {
        $validated = $request->validate([
            ...
            'detail_contact_heading' => ['required','string','max:180'],
            'detail_contact_text' => ['required','string','max:1500'],
            'detail_contact_button' => ['required','string','max:40'],
        ]);

        SiteContent::storeValues('events', $validated);

        return back()->with('success', 'Events page settings saved. Event records were not changed.');
    }
}